diff --git a//Users/soniasabet/live-switcher-dashboard/README.md b//Users/soniasabet/live-switcher-dashboard/README.md
--- a//Users/soniasabet/live-switcher-dashboard/README.md
+++ b//Users/soniasabet/live-switcher-dashboard/README.md
@@ -1,95 +1,60 @@
-<div align="center">
-  <img src="IMG_1495.PNG" alt="Sonia Sabet" width="160" style="border-radius: 50%;" />
+# Live Switcher Dashboard
 
-  <h1>Sonia Sabet</h1>
-  <p><strong>IT Support & Networking Specialist | Junior Web Developer (Aspiring)</strong></p>
+A lightweight real-time switcher control demo with a React frontend and a Node.js WebSocket server.
 
-  <p>
-    <a href="https://www.linkedin.com/in/sonia-sabet/">LinkedIn</a> •
-    <a href="https://inovnia.com/">Website</a> •
-    <a href="mailto:sonia.sabet@gmail.com">sonia.sabet@gmail.com</a>
-  </p>
+## What It Does
 
-  <p>
-    <img src="https://img.shields.io/badge/Location-Ottawa%2C%20ON-blue" alt="Location" />
-    <img src="https://img.shields.io/badge/Open%20to-Junior%20Web%20Developer%20%7C%20Application%20Support-success" alt="Open to roles" />
-  </p>
+- Shows current `Program` and `Preview` inputs
+- Supports `PVW`, `CUT`, `TAKE`, and `AUTO` actions
+- Streams live audio meter values
+- Syncs state across connected clients via WebSocket
 
-  <p>
-    <img src="https://skillicons.dev/icons?i=js,react,nextjs,html,css,py,github,nodejs,vscode&perline=9" alt="Tech Stack" />
-  </p>
-</div>
+## Tech Stack
 
-<hr/>
+- React + TypeScript (client)
+- Vite
+- Node.js
+- `ws` (WebSocket server library)
 
-<h2>About Me</h2>
-<p>
-  I am an IT support and networking specialist transitioning into web development.
-  I build React apps and Python automation scripts to solve real workflow problems.
-  I enjoy troubleshooting, writing clear documentation, and turning requirements into practical tools.
-</p>
+## Project Structure
 
-<h2>Core Skills</h2>
-<ul>
-  <li><strong>Web Development:</strong> HTML, CSS, JavaScript, React, Next.js (foundational), responsive UI</li>
-  <li><strong>Automation / Scripting:</strong> Python (utilities, data cleanup, workflow automation)</li>
-  <li><strong>Tools:</strong> Git/GitHub, VS Code, REST basics, Figma basics</li>
-  <li><strong>IT & Networking:</strong> Windows/Linux, Active Directory, Microsoft 365, VLAN/VPN, system administration fundamentals</li>
-</ul>
+```text
+live-switcher-dashboard/
+├─ client/   # React + Vite UI
+└─ server/   # WebSocket server
+```
 
-<h2>Featured Projects</h2>
+## Local Development
 
-<h3>1) Live Switcher Dashboard (React + Node.js)</h3>
-<p>
-  A lightweight real-time switcher control demo with a React frontend and a Node.js WebSocket server.
-</p>
-<ul>
-  <li>Shows current <code>Program</code> and <code>Preview</code> inputs</li>
-  <li>Supports <code>PVW</code>, <code>CUT</code>, <code>TAKE</code>, and <code>AUTO</code> actions</li>
-  <li>Streams live audio meter values and syncs state across connected clients</li>
-</ul>
-<p>
-  <strong>Tech:</strong> React, TypeScript, Node.js, WebSocket, CSS<br/>
-  <a href="https://github.com/soniasabet/live-switcher-dashboard">Repository</a>
-</p>
+1. Start the WebSocket server:
+```bash
+cd server
+npm install
+npm start
+```
 
-<h3>2) Log & CSV Cleaner (Python)</h3>
-<p>
-  A Python utility that cleans messy CSV/log data, removes duplicates, and outputs a structured report.
-</p>
-<ul>
-  <li>Detects invalid rows and standardizes formats</li>
-  <li>Removes duplicates and generates summary stats</li>
-  <li>Exports a clean file ready for analysis</li>
-</ul>
-<p>
-  <strong>Tech:</strong> Python<br/>
-  <a href="[REPO_LINK_2]">Repository</a>
-</p>
+2. Start the frontend:
+```bash
+cd client
+npm install
+npm run dev
+```
 
-<h3>3) Personal Portfolio (Next.js)</h3>
-<p>
-  A fast, responsive portfolio website showcasing projects and contact information.
-</p>
-<ul>
-  <li>Project cards with clear descriptions and links</li>
-  <li>Mobile-first layout and basic SEO</li>
-  <li>Simple structure that is easy to maintain</li>
-</ul>
-<p>
-  <strong>Tech:</strong> Next.js, JavaScript, HTML/CSS<br/>
-  <a href="[REPO_LINK_3]">Repository</a> • <a href="https://inovnia.com/">Live Site</a>
-</p>
+3. Open:
+`http://localhost:5173`
 
-<hr/>
+Server default WebSocket endpoint:
+`ws://localhost:3001`
+
+## Portfolio Project Summary
 
-<h2>How I Work</h2>
-<ul>
-  <li>Write clear, maintainable code with practical structure</li>
-  <li>Document projects with clean READMEs and step-by-step setup</li>
-  <li>Collaborate comfortably with technical and non-technical teams</li>
-</ul>
+**Live Switcher Dashboard** is a real-time control interface that simulates core production switching actions.
 
-<p align="center">
-  <em>Always learning, building, and improving.</em>
-</p>
+- Real-time `Program`/`Preview` sync across clients
+- Transition controls: `PVW`, `CUT`, `TAKE`, `AUTO`
+- Live audio meter updates over WebSocket
+- Clean UI built with reusable React components
+
+**Tech:** React, TypeScript, Node.js, WebSocket, CSS
+
+**Repository:** https://github.com/soniasabet/live-switcher-dashboard
